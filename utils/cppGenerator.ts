...
${keyStructs}
  ...
  // 9. Generate Documentation Header
  const buttonDocs = buttons.map((b, i) => {
    const shortDesc = b.shortPress.mode === 'key'
      ? `Key: ${b.shortPress.keyConfig.name.replace('KEY_', '')}`
      : `Text: "${b.shortPress.text.substring(0, 10)}..."`;
    return ` * Button ${i + 1}: GPIO ${b.pin} -> ${shortDesc}`;
  }).join('\n');

  const keypadDocs = enabledKeypads.map((kp, i) => `
 * Keypad ${i + 1} (${kp.name}):
 *   Rows (Outputs):    ${kp.rowPins.join(', ')}
 *   Cols (Inputs_PU):  ${kp.colPins.join(', ')}
 *   Wiring: Connect Rows to numbers 1-4 and Cols to 5-8 on standard 4x4 keypad`).join('\n');
